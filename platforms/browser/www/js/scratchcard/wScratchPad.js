(function ($) {
  ...
  ScratchPad.prototype = {
    _generate: function () {
      ...
      //this.$img = $('<img src=""/>').attr('crossOrigin', '').css({ 'z-index': '4000', position: 'absolute', height: '100%' });
      this.$img = $('<img name="imgPremioRasp" src=""/>').attr('crossOrigin', '').css({ 'z-index': '4000', position: 'fixed', width: '100%', height: 'auto' });

      //this.$scratchpad = $(this.canvas).css({ 'border-radius': '25px', 'z-index': '5000', position: 'absolute', width: '100%', height: '100%' });
      var widthCanvas = ($("#app").width()*0.898);
      var heightCanvas = '250px';
      this.$scratchpad = $(this.canvas).css({ 'border-radius': '25px', 'z-index': '5000', position: 'fixed', height: heightCanvas, width: widthCanvas });
      this.$scratchpad = $(this.canvas).attr('name', 'RaspeAqui');
      //this.$img = $('<img name="imgPremioRasp" src=""/>').addClass("areaRaspadinhaCanvasIMG");;
      //this.$scratchpad = $(this.canvas).addClass("areaRaspadinhaCanvas");;
